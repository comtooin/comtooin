...
const AdminHelpPage: React.FC<AdminHelpProps> = ({ isDialog = false, onClose }) => {
  ...
  const allSections = [
    {
      ...
      details: isCustomer 
        ...
        : [
            ...
            '사내 멤버 전원 실시간 알림 송출: 업무일지나 기술지원 건이 등록되면 사내 전체 엔지니어 멤버들의 스마트폰 및 PC로 실시간 웹 푸시 알림이 즉각 전송되어 팀원 간 신속한 상황 공유가 가능합니다.',
    ...
    {
      id: 'messenger',
      title: '메신저 (실시간 1:1 및 비회원 채널)',
      shortTitle: '메신저',
      icon: <ForumIcon color="primary" />,
      desc: isCustomer
        ? '컴투인 엔지니어와 실시간 1:1 대화를 통해 장애 문의 및 기술지원을 신속히 소통하는 방법입니다.'
        : '거래처 및 비회원 고객들과 1:1 전용 채널을 통해 실시간 기술지원 및 업무 피드백을 진행하는 방법입니다.',
      details: isCustomer
        ? [
            '1:1 대화방을 통한 실시간 기술 문의: 로그인 후 사이드바 메뉴의 [메신저] 단추를 누르면 거래처 전용 대화방으로 즉시 진입하여 엔지니어와 실시간 소통을 나눌 수 있습니다.',
            '철저한 기밀 대화방 보안 격리: 다른 거래처와의 정보 혼선 및 유출을 방지하기 위해, 오직 본인 거래처의 대화방만 안전하게 보안 격리 노출됩니다.',
            '비회원 간편 1:1 대화방 개설 링크: 별도 회원가입이나 로그인 없이도 전용 URL 링크(/messenger/...)를 통해 성함과 연락처를 입력하고 개인정보 동의 후 1:1 지원 대화방을 즉시 개설할 수 있습니다.',
            '실시간 엔지니어 푸시 및 이메일 호출: 대화방이 생성되거나 새 메시지를 작성하면 컴투인 엔지니어 전원의 스마트폰과 이메일로 즉시 알림이 도달하여 신속한 조치 답변을 받으실 수 있습니다.'
          ]
        : [
            '거래처 및 비회원 실시간 대화방 관리: 사이드바 [메신저] 메뉴에서 계약 거래처 전용 채널 및 비회원이 개설한 1:1 기술지원 대화방을 실시간 모니터링하고 응대할 수 있습니다.',
            '신규 1:1 대화방 개설 시 이중 알림(푸시+이메일): 거래처나 비회원이 새 대화방을 생성하는 즉시 사내 직원 전원의 스마트폰/PC 웹 푸시 팝업과 이메일 [수신함]으로 개설 안내 메일이 동시 발송됩니다.',
            '실시간 메시지 푸시 알림: 고객이 대화창에 문의를 남기면 사내 직원 전원의 스마트폰 잠금화면으로 메시지 알림 팝업이 울려 현장에서도 지체 없이 답변할 수 있습니다.',
            '대화방 검색 및 편리한 실시간 채팅: 거래처명 검색을 통해 과거 상담 이력을 빠르게 찾을 수 있으며, [기술지원 요청] 바로가기를 통해 대화 중 접수된 장애를 즉시 업무일지에 연동할 수 있습니다.'
          ]
    },
    {
      id: 'profile',
      title: '내 정보 및 기기 알림 관리',
      shortTitle: '알림 / 프로필',
      icon: <PersonIcon color="primary" />,
      desc: isCustomer
        ? '내 회원 정보를 조회·수정하고, 스마트폰/PC의 실시간 알림 수신 상태를 설정하는 방법입니다.'
        : '담당자 정보 및 비밀번호를 관리하고, 스마트폰 모바일 백그라운드 푸시 알림을 제어하는 방법입니다.',
      details: isCustomer
        ? [
            '기본 정보 및 연락처 수정: 상단 프로필 메뉴를 클릭하여 이름, 이메일, 담당자 연락처를 확인하고 필요시 즉시 수정·저장할 수 있습니다.',
            '원클릭 기기 푸시 알림 초기화/끄기: 스마트폰이나 PC에서 알림을 끄고 싶을 때 [이 기기 푸시 알림 초기화 / 끄기] 버튼을 누르면 즉시 알림 구독이 해제되고 안전하게 초기화됩니다.',
            '푸시 알림 다시 켜기: 알림을 다시 받고 싶다면 [푸시 알림 다시 켜기 / 등록] 버튼을 눌러 언제든 간편하게 알림을 재활성화할 수 있습니다.'
          ]
        : [
            '사내 멤버 모바일 백그라운드 알림 영구 유지: 사내 직원은 스마트폰에서 로그아웃하거나 브라우저를 종료해도 푸시 구독이 DB에 보존되어, 카카오톡처럼 잠금화면으로 24시간 실시간 업무/메신저 알림이 계속 들어옵니다.',
            '스마트폰 OS별 알림 수신 방법: 안드로이드는 주소창 🔒 자물쇠 아이콘에서 [알림 허용]을 켜주시면 되며, 아이폰(iOS)은 Safari 하단 공유 버튼(⬆️) ➔ [홈 화면에 추가] 후 바탕화면 앱으로 1회 접속하시면 푸시 알림이 완벽히 활성화됩니다.',
            '원클릭 기기 푸시 알림 초기화/끄기: 사용 중인 스마트폰이나 PC에서 알림을 중단하고 싶을 때는 [내 정보/프로필] 페이지의 [이 기기 푸시 알림 초기화 / 끄기] 버튼을 눌러 간편하게 초기화할 수 있습니다.',
            '공용 PC 보안 로그아웃: 공용 PC에서는 로그아웃 시 해당 브라우저의 푸시 ID가 DB에서 안전하게 자동 정리되므로 계정 보안이 철저히 유지됩니다.'
          ]
    },
    {
      id: 'customer',
  ...
  const sections = isCustomer
    ? [
        ...
        allSections.find(sec => sec.id === 'profile')
