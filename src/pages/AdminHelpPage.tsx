  ...
  const allSections = [
    {
      ...
      details: isCustomer 
        ? [
            '실시간 통계 및 그래프 확인: 대시보드 상단 수치 카드를 통해 진행률을 확인하고, 거래처별 비중 차트와 연간 월별 막대그래프를 통해 우리 회사의 유지보수 고장 및 장애 처리 비중을 분석할 수 있습니다.',
            '상세 유지보수 내역 조회: 하단 테이블을 통해 지금까지 조치 완료된 컴퓨터 사양 교체 내용이나 현장 점검 장애 조치 내역을 상세히 검색 및 열람할 수 있습니다.',
            '엑셀 다운로드: 하단 테이블에서 필요한 기간을 설정한 후 [리포트 다운로드(CSV)]로 우리 회사에 등록된 유지보수 실적 로그를 엑셀 파일로 소장할 수 있습니다.'
          ]
        : [
